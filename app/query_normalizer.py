...
def normalize_query(text: str) -> str:
    ...
    # GROBE STT-FEHLER FRÜH ABFANGEN

    # Bridget Riley Problemfälle
    q = re.sub(r"\bunlimited bridge to try me\b", "wann lebte bridget riley", q)
    q = re.sub(r"\bcant leave the bridge dryly\b", "wann lebte bridget riley", q)
    q = re.sub(r"\bcan't leave the bridge dryly\b", "wann lebte bridget riley", q)
    ...
    q = re.sub(r"\bdann lebt bridget riley\b", "wann lebte bridget riley", q)
    ...
    q = re.sub(r"\bwann liebte bridget riley\b", "wann lebte bridget riley", q)

    # Victor Vasarely Problemfälle
    ...
    q = re.sub(r"\bwasarew\b", "vasarely", q)
    q = re.sub(r"\bwasarev\b", "vasarely", q)

    # Kandinsky Problemfälle
    q = re.sub(r"\bbasilikum\b", "kandinsky", q)

    # Yabla Problemfälle
    q = re.sub(r"\bjoblücke\b", "yabla", q)
    q = re.sub(r"\bjoblucke\b", "yabla", q)
    q = re.sub(r"\bjobluke\b", "yabla", q)
    ...
    q = re.sub(r"\bkandiski\b", "kandinsky", q)
    ...
    # Allgemeine Frageformen
    q = re.sub(r"^lebte ([a-zäöüß][a-zäöüß\s\-]+)$", r"wann lebte \1", q)
    q = re.sub(r"^liebte ([a-zäöüß][a-zäöüß\s\-]+)$", r"wann lebte \1", q)
    q = re.sub(r"^dann lebte ([a-zäöüß][a-zäöüß\s\-]+)$", r"wann lebte \1", q)
    q = re.sub(r"^dann lebt ([a-zäöüß][a-zäöüß\s\-]+)$", r"wann lebte \1", q)
    q = re.sub(r"^wann liebte ([a-zäöüß][a-zäöüß\s\-]+)$", r"wann lebte \1", q)
